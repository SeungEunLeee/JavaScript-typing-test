...
let timeElapsed = 0 // 얼만큼 시간이 지났는지
let totalErrors = 0
let errors = 0
let accuracy = 0
let characterTyped = 0
let currentQuote = ""
let quoteNo = 0 // quoteNumber
let timer = null
...
function startGame() {
    ...
    // 타이머 시작
    clearInterval(timer)
    timer = setInterval(updateTimer, 1000) // 1초에 한번씩 updatTimer 함수 호출
}
...
function finishGame() {
    clearInterval(timer)
     ...
     // wpm    
     // word per minutes 분당 몇개의 단어를 작성했는지
 
     // cpm 
     // characters per minutes 분당 몇개의 알파벳을 작성했는지
